...
import ca.bc.gov.pssg.docmerge.model.MergeDoc;
import ca.bc.gov.pssg.docmerge.utils.DDXUtils;
import ca.bc.gov.pssg.docmerge.utils.DocMergeConstants;
import ca.bc.gov.pssg.docmerge.utils.PDFBoxUtilities;
 ...
 */
@Service
@PropertySource(ignoreResourceNotFound = false, value = "classpath:docmerge.properties")
public class MergeServiceImpl implements MergeService {

	private final Logger logger = LoggerFactory.getLogger(MergeServiceImpl.class);
	
	@Value("${docmerge.aem.endpoint}")
	private String aemEndpoint;
	
	@Value("${docmerge.aem.user}")
	private String aemUser;
	
	@Value("${docmerge.aem.password}")
	...
	public DocMergeResponse mergePDFDocuments(DocMergeRequest request, String correlationId) throws MergeException {
		...
		try {
			...
			connectionProps.setProperty(ServiceClientFactoryProperties.DSC_CREDENTIAL_USERNAME, aemUser);
			connectionProps.setProperty(ServiceClientFactoryProperties.DSC_CREDENTIAL_PASSWORD, aemPassword);
			...
			Collections.sort(request.getDocuments(), new Comparator<ca.bc.gov.pssg.docmerge.model.Document>() {
			    public int compare(ca.bc.gov.pssg.docmerge.model.Document d1, ca.bc.gov.pssg.docmerge.model.Document d2) {
			        return d1.getOrder().compareTo(d2.getOrder());
			    }
			});
			
			// For each document, check if XFA and convert to PDF/A if requested via option. 
			for (ca.bc.gov.pssg.docmerge.model.Document doc: request.getDocuments()) {
